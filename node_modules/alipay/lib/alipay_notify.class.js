...
AlipayNotify.prototype.verifyNotify = function(_POST, callback){
    if(Object.keys(_POST).length == 0) {//判断POST来的数组是否为空
        ...
        //生成签名结果
        var isSign = this.getSignVeryfy(_POST, _POST["sign"], false);
...
AlipayNotify.prototype.verifyReturn = function(_GET, callback){
    if(Object.keys(_GET).length == 0) {//判断POST来的数组是否为空
        ...
        //生成签名结果
        var isSign = this.getSignVeryfy(_GET, _GET["sign"], true);
 ...
 * @param sign 返回的签名结果
 * @param isSort 时候对签名进行排序
 * @return 签名验证结果
 */
AlipayNotify.prototype.getSignVeryfy = function(para_temp, sign, isSort){
    //除去待签名参数数组中的空值和签名参数
    var para_filter = core_funcs.paraFilter(para_temp);

    // 构建签名字符串
    var prestr = querystring.unescape(querystring.stringify(para_filter));
